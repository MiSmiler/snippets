...
test("one divider: the block above ends right before the divider row", () => {
  // CDOC below: alpha, blank, --- (line 3), beta, gamma.
  const block = blockAt(makeState("alpha\n\n---\nbeta\ngamma"), 2)!;
  assert.deepEqual({ from: block.from, to: block.to }, { from: 0, to: 7 });
...
test("two dividers yield three blocks (blank margins belong to their block)", () => {
  const doc = "one\n\n---\ntwo\n\n---\nthree";
...
// Block 1 is [0, 7) "alpha\n\n"; block 2 is [11, 21) "beta\ngamma".
const CDOC = "alpha\n\n---\nbeta\ngamma";

test("first press inside a block selects that block", () => {
...
test("pressing again after the block is selected expands to the whole document", () => {
  ...
  assert.deepEqual(target(CDOC, 7, 0), { from: 0, to: CDOC.length });
});
