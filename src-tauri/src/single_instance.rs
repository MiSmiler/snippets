...
//
// This module is a trimmed, single-file adaptation of the Windows
// implementation of `tauri-plugin-single-instance` (v2.4.3). It was moved
// in-tree because the plugin offers no way to customize the mutex name, and
// we need the guard to be scoped per build profile: a debug build and the
// release build must be able to run side by side, each still enforcing its
...
struct UserData<R: Runtime> {
    app: AppHandle<R>,
    callback: Box<dyn FnMut(&AppHandle<R>, Vec<String>, String) + Send + Sync>,
}
...
/// Install the single-instance guard. A second instance of the same build
/// profile finds the hidden event window of the first instance, forwards its
/// command line, and exits; the first instance's callback (which focuses the
/// main window) is invoked with the forwarded data.
pub fn init<R: Runtime, F>(app: &App<R>, callback: F) -> tauri::Result<()>
where
    F: FnMut(&AppHandle<R>, Vec<String>, String) + Send + Sync + 'static,
{
    // Scope the guard per build profile so debug and release builds coexist.
    ...
    if unsafe { GetLastError() } == ERROR_ALREADY_EXISTS {
        // A first instance is running: ask it to focus itself, then quit.
        unsafe {
...
/// Release the mutex and destroy the hidden event window on exit.
    ...
    unsafe {
        ...
        let hwnd = CreateWindowExW(
            WS_EX_NOACTIVATE
                | WS_EX_TRANSPARENT
                | WS_EX_LAYERED
                // WS_EX_TOOLWINDOW prevents this window from ever showing up
                // in the taskbar.
                | WS_EX_TOOLWINDOW,
            class_name.as_ptr(),
            window_name.as_ptr(),
        ...
        SetWindowLongPtrW(
            hwnd,
            GWL_STYLE,
            // The window technically has to be visible to receive WM_PAINT
            // messages (used for delivering events during resizes), but it is
            // not displayed to the user because of the LAYERED style.
