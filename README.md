...
- 分割线预览：单独成行的 `---` 渲染为贯穿横线（代码块 / 标题下划线内不生效），并按分割线把笔记划分为多个 block（分割线归属其上方 block；光标在 block 内时 `Ctrl+A` 选中文字内容，在分割线上时选中整个 block）
...
- 备份 / 迁移：拷走 exe 整个文件夹
- 设置（字号、主题、分词引擎、上次会话）存 exe 旁 `settings.json`，删除该文件即恢复默认
- `Ctrl+,` 设置里可切换中文按词移动/删除所用的分词引擎：**jieba · Standard**（默认，内置词典 + HMM，能识别人名 / 新词）、**jieba · Fine**（在长词内部也停顿，移动步长更细）、**System default**（WebView 内置 ICU，旧行为）；切换即时生效，无需重启
...
| `Ctrl+Z` / `Ctrl+Y` | 撤销 / 重做 |
| `Ctrl+A` | 选中光标所在 block（`---` 为分割线；无分割线时整篇是一个 block）；再按一次全选全文 |
| `Enter` | 列表行内回车自动延续 `- ` 前缀 |
